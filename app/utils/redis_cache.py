...
def get_from_cache(redis_client, cache_key: str) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(f"Corrupted cache data for key '{cache_key}', clearing: {str(e)}")
...
def store_in_cache(redis_client, cache_key: str, data: list[dict], expiration: int) -> None:
    ...
    try:
        ...
        logger.info(f"✅ music catalog cached for {expiration} seconds")
    except Exception as e:
        logger.warning(f"⚠️ Failed to cache music catalog: {str(e)}")